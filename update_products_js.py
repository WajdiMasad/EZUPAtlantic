...
filepath = 'e:\\EZUP\\js\\products.js'
with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Update categoryNames
...
content = content.replace('if (catFilter && p.category !== catFilter) return false;', 
                          'if (catFilter && p.category !== catFilter && p.subcategory !== catFilter) return false;')

# 3. Update category badge on cards to show subcategory
content = content.replace('const catName = categoryNames[p.category] || \'Product\';',
                          'const catName = categoryNames[p.subcategory] || categoryNames[p.category] || \'Product\';')

with open(filepath, 'w', encoding='utf-8', newline='\n') as f:
    f.write(content)
print("Updated js/products.js")
